...
class _ActivityManagementPageState extends State<ActivityManagementPage> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Erreur catégories : $e'),
  ...
  void _showActivityDialog({AdminActivity? activity}) {
    ...
    final titleCtrl = TextEditingController(
      text: isEditing ? activity.title : "",
    );
    final descCtrl = TextEditingController(
      text: isEditing ? activity.shortDescription : "",
    );
    final contentCtrl = TextEditingController(
      text: isEditing ? activity.content : "",
    );
    final urlCtrl = TextEditingController(
      text: isEditing ? activity.activityUrl : "",
    );
    final imgCtrl = TextEditingController(
      text: isEditing ? activity.imageUrl : "",
    );
    ...
    int currentUserId = isEditing
        ? activity.idUtilisateur
        : 1; // ID admin par défaut

    showDialog(
      context: context,
      barrierDismissible:
          false, // Force l'utilisateur à cliquer sur Annuler ou Valider
      builder: (dialogContext) => AlertDialog(
        backgroundColor: Colors.white,
        title: Text(
          isEditing ? "Modifier l'activité" : "Nouvelle activité",
          style: AppTextStyles.titleStyleH2.copyWith(
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextFormField(
                    controller: urlCtrl,
                    decoration: const InputDecoration(
                      labelText: "URL de l'activité (Optionnel)",
                    ),
                  ),
                  TextFormField(
                    controller: imgCtrl,
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            icon: Icon(Icons.refresh, color: AppColors.tropicalTeal),
            onPressed: _refresh,
          ),
        ],
      ),
        ...
        builder: (context, snapshot) {
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, i) => ListTile(
              ...
              subtitle: Text(
                acts[i].shortDescription,
                style: AppTextStyles.textStyleRegularBackground,
              ),
              trailing: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
